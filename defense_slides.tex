...
\begin{frame}{A challenger appears: the I-mode}
 ...
 \begin{itemize}
  ...
  \vspace{0.5em}
  \item pedestal regulated by weakly-coherent mode
 \end{itemize}
...
\begin{frame}{Kinetic-ballooning mode (KBM) turbulence limits gradient}

 \large
 Above critical gradient, electromagnetic turbulence drives rapidly-saturating transport.
 
 \vspace{0.5em}
 Locally at outboard midplane, magnetic shear stabilizes mode such that $\alpha_c \sim 1/s^{1/2}$.  Shear trends with current as $1/\langle j \rangle \rightarrow 1/\beta_{p,ped}$ for bootstrap current: thus $\alpha_c \sim \beta_{p,ped}^{1/2}$.
 
 \vspace{0.5em}
 Since $\alpha_c \sim \beta_{p,ped}/\Delta$, this implies a width constraint $\Delta \sim \beta_{p,ped}^{1/2}$
 
 \vspace{0.5em}
 onset threshold correlated with infinite-$n$ ideal ballooning mode stability $\rightarrow$ readily calculated
...
\begin{frame}{What does this get us?}
 ...
 \begin{itemize}
  ...
  \vspace{0.5em}
  \item L-mode density profile $\rightarrow$ fuel core via turbulent particle pinch, despite neutral-opaque SOL: desirable for ITER
 \end{itemize}
